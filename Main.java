...
public class Main {
   /**
    * main METHOD is teh entry point to the program 
   ...
   public static void main(String []args) {
      ...
      // Methods with a RETURN VALUE (non void) mean you need to "do something" with that output 
      Math.abs(-41); // works, but can't see results 
      // You could store the results in a variable 
      int absValue = Math.abs(-35); 
      System.out.println(absValue); 

      // OR you can display it directly 
      System.out.println(Math.abs(-99.8)); 

      // .abs is an OVERLOADED method 
      // meaning there are multiple versions of the smae method, with the same name but different parameters 
      ...
      // Math.sqrt takes one argument returns the square root 
      double root99 = Math.sqrt(99.0);
      System.out.println(root99);

      //Math.random() returns a double between [0.0, 1.0) -> is not included 
      double randNum = Math.random(); 
      System.out.println(randNum); 

      // If you want to generate a number in a different range you just adjust the output of math.random()
      ...
      // GENERAL FORMAT: (int) (Math.random() * max) + min 
      ...
      // 2. They made a CHORTCUT to set up Strings 
      String str2 = "AP Java"; 
      ...
      // .substring() is an OVERLOADED method (2 versions) that returns part of a larger String 
      String message = "APCS is awsome!"; 
      
      String firstWord = message.substring(0, 4); 
      System.out.println(firstWord); 
      // .substring(int from, int to) DOES NOT INCLUDE the character at index to 
      ...
      System.out.println("Does m1 = m2? " + areTheyEqual); // false 
      // order doesn't matter here 
      areTheyEqual = message2.equals(message1); 
      System.out.println(areTheyEqual); 

      // you can compare String variables to a String LITERAL 
      ...
      // returns an INT representing the alphabetical order 
      ...
      // negative numbers mean the OTHER string comes AFTER the original/calling string, message1 
